...
describe("MyERC721", function () {
    ...
    describe("初期流通量とNFT作成のテスト", function () {
        ...
        it("account1からは作成ができないことの確認", async function () {
            ...
            await expect(MyERC721.connect(account1).safeMint(account1.address, 'https://example.com/nft1.json')).to.be.revertedWith(/ Access Control: account.* is missing role .*/);
            }); 
        });
        ...
        describe("MyERC721をtransferするテスト", function () {
            it("MyERC721をtransferするテスト", async function () {
                ...
                // NFTの保有者が owner に変更されることの確認
                expect(await MyERC721. ownerOf(tokenId)).to.equal (owner.address);
            });
            it("account1から owner 保有のNFTはtransferができないことの確認", async function () {
                const { MyERC721, owner, account1 } = await loadFixture(deployFixture); // ownerを保有者とするNFTを作成する
                const txResp = await MyERC721.safeMint (owner.address, 'https://example. com/nft1.json');
                // Transfer イベントからtokenIdを特定する
                const logs = await MyERC721.queryFilter (MyERC721. filters. Transfer());
                const tokenId = logs.find( log => log.transactionHash == txResp.hash)!.args[2];
                // hardhat-chai-matcherの機能を使ってTxが意図したエラーでRevert されること を確認する
                await expect(MyERC721.connect(account1).transferFrom(owner.address, account1.address, tokenId)).to.be.revertedWith('ERC721: caller is not token owner or approved');
            });
            it("NFT保有者がapproveすればaccount1からもNFTをtransfer できることの確認", async function () {
                const { MyERC721, owner, account1 } = await loadFixture (deployFixture); // ownerを保有者とするNFTを作成する
                const txResp = await MyERC721.safeMint (owner.address, 'https://example.com/nft1.json');
                // Transfer イベントからtokenIdを特定する
                const logs = await MyERC721.queryFilter (MyERC721. filters. Transfer());
